...
BEGIN
    ...
    WHILE i < 10 DO
        INSERT INTO game(title, description, releaseDate, developer, publisher, genre, price)
        VALUES (CONCAT('Game', i), 'This is a random game.', CURDATE(), 'Random Developer', 'Random Publisher', 'Random Genre', RAND() * 100);
        SET i = i + 1;
    END WHILE;
END//
DELIMITER ;
...
END//
DELIMITER ;
-- Call InsertIntoUserAndUserFriendship procedure
CALL InsertIntoUserAndUserFriendship(1, 'UserName', 'email@example.com', 'password', '2023-01-01', 1, 1, 2);


DELIMITER //
CREATE PROCEDURE InsertIntoUserAndUserFriendship
(IN userId INT, IN userId2 INT)
BEGIN
    DECLARE userName_1 VARCHAR(255);
    DECLARE userName_2 VARCHAR(255);

    SELECT userName INTO userName_1 FROM user WHERE id = userId;
    SELECT userName INTO userName_2 FROM user WHERE id = userId2;
    IF (userName_1 IS NOT NULL AND userName_2 IS NOT NULL) THEN
        INSERT INTO user_friendship(user_id, user_2_id, userName_1, userName_2)
    VALUES (userId, userId2, userName_1, userName_2);
        SELECT 'Successful insert' AS Message;
    ELSE
        SELECT 'UserIds do not exist' AS Message;
    END IF;
END //
DELIMITER ;
...
-- Створення тригера, який перевіряє що рейтинг входить у допустимі межі(0,10)
DELIMITER //
CREATE TRIGGER check_rating_validity
BEFORE INSERT ON review
FOR EACH ROW
BEGIN
    IF NEW.rating < 0 OR NEW.rating > 10
    THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Rating out of range (0,10)';
    END IF;
END //
DELIMITER ;


-- Створення тригера, який ставить нижню допустиму межу року народження 
DELIMITER //
CREATE TRIGGER CheckDateOfBirth
BEFORE INSERT ON user
FOR EACH ROW
BEGIN
    IF YEAR(NEW.dateOfBirth) <= 1900 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Date of birth should be after 1900.';
    END IF;
END//
DELIMITER ;


-- Створення тригера, який ставить ліміт на ціну транзакції
DELIMITER //
...
BEGIN
    IF NEW.amount >= 1000000 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Transaction amount limit exceeded';
    END IF;
END//
DELIMITER ;
